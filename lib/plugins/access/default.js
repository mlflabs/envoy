...
const canReadById = async (id, user) =>{
  try {
    console.log('CanReadByID: ', id, user);
...
const accessByUser = (doc, user) => {
  console.log('AccessByUser:: ', doc, user);
  const users = extractUsers(doc);
  if(users.includes(user.name)){
...
const authCheckStream = function(user, removeDoc) {
  console.log('authRemover: ', user, removeDoc);
  let firstRecord = true;
  
  
  const stripAuth = function (obj, user, removeDoc) {
    console.log('stripAuth: ', obj, user, removeDoc);
    let addComma = false;
    let chunk = obj;
