...
const processBulk = async (res) => {
  console.log('-- ChangesDB: processBulk');  
  
  //first we need to save changes without doc opbjet
  //then we save the docs
  const changes = res.map(row => {
    delete row.doc;
    return doc;
  });
  console.log(changes);
  await changesInsertBulk(changes);
  await docsInsertBulk(changes);
...
const getLatestChangeSeq =  async (opts) => {
