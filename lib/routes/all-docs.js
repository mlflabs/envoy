...
const getSelector = (ownerid, query) => {
  delete query.keys
  delete query.key
  // TODO: cant use addownerid query.startkey = access.addOwnerId('', ownerid)
  query.endkey = query.startkey.replace(access.delimiter, access.delimiter + 'z')
  query.descending = false
  console.log('********************* IMP TODO: GET selector query:  ', query);
  return query
}
...
router.get('/' + app.dbName + '/_all_docs', auth.isAuthenticated, (req, res) => {
  if (req.query.keys) {
    ...
    */
    ...
    // TODO: need to allow users just to load all its docs without
    // providing ids, but its not needed to fulfill couchdb replication
    // standard
    // for now just return valid result with 0 docs
    return res.send({
      total_rows: 0,
      rows: [],
      offset: 0
