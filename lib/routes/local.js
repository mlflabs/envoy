...
router.get('/s' + app.dbName + '/_local/:key', auth.isAuthenticated, async (req, res) => {
  
  const key = req.params.key;
  const user = req.session.user.name;
  
  // see if we have this session key, if not just send error
  const rec = changesdb.getSyncUserKey('_local_'+user+'_'+key);
...
router.put('/d' + app.dbName + '/_local/:key', auth.isAuthenticated, (req, res) => {
  app.cloudant.request({
    db: app.dbName,
    path: access.addOwnerId('_local/' + req.params.key, req.session.user.name),
    method: 'PUT',
